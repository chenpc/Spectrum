...
| `Services/GyroCore.swift` | `final class`：dlopen libgyrocore_c.dylib，FFI 矩陣計算（one-shot API） |
| `Services/GyroConfig.swift` | `GyroConfig` 配置結構 + `GyroCoreProvider` 協定 |
...
**GyroConfig**：Codable 結構體，22 個可調參數（readoutMs、smooth、gyroOffsetMs、integrationMethod、imuOrientation、fov、lensCorrectionAmount、zoomingMethod、zoomingAlgorithm、adaptiveZoom、maxZoom、maxZoomIterations、useGravityVectors、videoSpeed、horizonLockEnabled、horizonLockAmount、horizonLockRoll、perAxis、smoothnessPitch/Yaw/Roll、lensDbDir）。JSON CodingKeys 使用 snake_case。
...
**C FFI 函式**（one-shot API）：
```c
void* gyrocore_load(const char* video_path, const char* lens_path, const char* config_json);
int   gyrocore_get_params(void* handle, void* out_buf);  // 96 bytes
int   gyrocore_get_frame(void* handle, uint32_t frame_idx, float* out_buf);
int   gyrocore_get_frame_at_ts(void* handle, double ts_sec, float* out_buf);
void  gyrocore_get_lens_info(void* handle, char* buf, int len);
void  gyrocore_free(void* handle);
```

**GoPro Rolling Shutter 處理**：gyro-wrapper 偵測 `detected_source.starts_with("GoPro")`，自動跳過 RS correction（GoPro ISP 已內部處理 rolling shutter）。
...
offset 40: float32[12] k (distortion coefficients)
offset 88: int32 distortion_model (0=None 1=OpenCVFisheye 3=Poly3 4=Poly5 7=Sony)
offset 92: float32 r_limit
```

**get_frame 輸出**：`row_count × 14 + 9 floats`，每行 = `[mat3×3(9), sx, sy, ra(IBIS), ox, oy(OIS)]`，尾部 9 floats = per-frame lens params
...
computeMatrixAtTime(timeSec) → (UnsafeBufferPointer<Float>, changed: Bool)?
...
scannedThisSession: Set<String>         ← 本次 session 已掃描路徑（invalidate 時清除）
entries(for: parentPath) → [FolderListEntry]?
entry(forChildPath:, underParent:) → FolderListEntry?
isScannedThisSession(_:) → Bool         ← 檢查是否已掃描（避免重複 I/O）
setEntries(_:, for:)                    ← 同時標記為已掃描
invalidate(parentPath:)                 ← 清除快取 + 移除已掃描標記
clear()
...
```
isActive, label, progressTotal, progressDone: 掃描/複製進度
doneMessage: String?                    ← 完成訊息（10 秒後自動清除）
globalLabel: String?                    ← 背景任務狀態（如 folder tree prefetch）

begin(_:)                               ← 啟動 indeterminate 任務
begin(_:, total:)                       ← 啟動 determinate 任務
update(done:, label:)
finish(_:)                              ← 完成，設 doneMessage + 10s 計時器
setGlobal(_:)                           ← 設定進行中背景任務標籤
finishGlobal(_:)                        ← 完成背景任務 + 10s 計時器
isVisible: Bool                         ← isActive || doneMessage != nil || globalLabel != nil
```
...
gyroSI: Double                              // Gyro Stability Index (RMS rotation)
gyroStabEnabled, gyroAvailable: Bool
diagnosticsEnabled: Bool
layerColorspaceInfo, decodeColorspaceInfo, pixelFormatInfo, colorSpaceInfo: String
codecInfo: String?
isAVFLayerMode: Bool
```
...
│       │   └── detail:
│       │       ├── searchText 非空 → SearchResultsView（照片+資料夾搜尋結果）
│       │       ├── 有 detailPhoto →
...
- **searchText**: `.searchable()` 搜尋欄文字；非空時顯示 SearchResultsView 取代正常內容
...
- **搜尋**: `.searchable(placement: .toolbar)` → SearchResultsView 同時搜尋照片檔名與資料夾名稱
...
- **狀態列**: StatusBarModel 統一顯示掃描/複製/貼上/匯入進度，完成後 10 秒自動消失
- **刪除**: Delete/Forward Delete 鍵，支援多選批量刪除；先嘗試 trashItem，失敗時 fallback 到 removeItem（永久刪除，適用於無垃圾桶的網路磁碟）
...
- HDR/SDR 切換 pill + GYRO toggle pill（on/off）
...
使用 `gyrocore_*` one-shot C API（GyroFlowCore 增量 API 已移除）。
...
GoPro ISP 在硬體層面處理 rolling shutter，gyroflow-core 會清除 `frame_readout_time`。gyro-wrapper 額外偵測 `detected_source.starts_with("GoPro")`，確保不會套用外部 RS 校正（避免邊緣變形抖動）。
...
| Integration Method | `gyroIntegrationMethod` | -1 (Auto) |
| IMU Orientation | `gyroImuOrientation` | "" (Auto) |
...
- **FolderListCache**：JSON 持久化 + session-level 去重，同路徑每 session 只掃一次；FolderMonitor invalidate 觸發重新掃描
