...
- **Spectrum**: `PhotoDetailView.startGyro()` → `MPVController.startGyroStab()` → `GyroCore.start()` → `gyrocore_load()`
- **Gyroflow**: `PhotoDetailView.startGyro()` → `MPVController.startGyroStabGyroflow()` → `GyroFlowCore.start()` → `gyroflow_create()` → `gyroflow_load_video()` → `gyroflow_set_param()` × N → `gyroflow_recompute()`

兩者最終都透過 `GyroCoreProvider` protocol 提供 `computeMatrixAtTime(timeSec)` 給 `MPVOpenGLLayer.draw()`，GPU warp shader 完全不知道底層是哪個引擎。Rust 側共用同一個 `libgyrocore_c.dylib`（兩組 API 並存）。
...
| 矩陣查詢 | 渲染線程（CAOpenGLLayer.draw） | 渲染線程 |
...
- **GPU warp shader** — WarpShader.swift 中的 6 個 GLSL 著色器
- **matTex 格式** — width=4 RGBA32F 紋理（每行 = 3×3 矩陣 + IBIS + OIS）
...
| `Spectrum/Views/Detail/MPVOpenGLLayer.swift` | GPU draw() pipeline, matTex upload |
| `Spectrum/Views/Detail/WarpShader.swift` | GLSL 著色器 |
| `Spectrum/Views/Detail/MPVController.swift` | 生命週期管理（start/stop/poll） |
